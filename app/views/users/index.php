<?php $pageTitle = 'Users'; ?>
<div class="topbar"><div class="greeting"><h1>Users</h1><p>Manage system users (Admin)</p></div>
    <a href="/users/create" style="color: #b8943c; background: rgba(184,148,60,0.1); padding: 0.5rem 1rem; border-radius: 8px; text-decoration: none;"><i class="fas fa-plus"></i> Add User</a>
</div>
<div class="table-section reveal">
    <div class="header"><h3>All Users (<?= count($users) ?>)</h3>
        <input type="text" name="search" placeholder="Search users..." value="<?= sanitize($search) ?>" style="padding: 0.4rem 0.8rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06); background: rgba(0,0,0,0.2); color: #f5f0eb;">
    </div>
    <table>
        ...
        <tbody>
            <?php if (empty($users)): ?>
                <tr><td colspan="8" style="text-align: center; color: rgba(245,240,235,0.3);">No users found</td></tr>
            <?php else: ?>
                <?php foreach ($users as $u): ?>
                    <tr>
                        <td><?= sanitize($u['first_name'] . ' ' . $u['last_name']) ?></td>
                        <td><?= sanitize($u['employee_id'] ?? '-') ?></td>
                        <td><?= sanitize($u['email'] ?? '-') ?></td>
                        <td><?= sanitize($u['phone'] ?? '-') ?></td>
                        <td><?= sanitize($u['department_name'] ?? '-') ?></td>
                        <td><span class="status <?= $u['role'] ?>"><?= ucfirst($u['role']) ?></span></td>
                        <td><span class="status <?= $u['status'] ?>"><?= ucfirst($u['status']) ?></span></td>
                        <td><a href="/users/<?= $u['id'] ?>/edit" style="color: #f59e0b;"><i class="fas fa-edit"></i></a></td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        </tbody>
